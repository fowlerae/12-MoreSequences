...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(75.0, 200.0),  # All numbers are approximate.
                rg.Point(25.0, 200.0)]
    circle = rg.Circle(rg.Point(50, 200), 25)
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 80), 50)
    draw_points_on_circle(window, circle, 6, 'blue')
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    # Tests 3:
    title = 'POLYGON tests 3:  8 segments with thick blue lines;  20 with medium red lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(200, 310), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 8, 'green', 20)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        ...
        if k < number_of_segments - 1:
            line = rg.Line(points[k],points[a])
        else:
...
def run_test_fancy_polygon():
    ...
    # Test 3 (on another window):
    title = 'FANCY POLYGON test 4:  40 blue lines on white circle, hops = 4.'
    window = rg.RoseWindow(600, 500, title)

    circle = rg.Circle(rg.Point(300, 250), 200)
    circle.fill_color = 'white'
    fancy_polygon(window, circle, 40, 4, 'blue', 1)
    window.close_on_mouse_click()
